...
- 把你查到的陣營「翻譯成對獸首的態度」:查到某人是好人 → 就順著、支持「他力挺要保的獸首」;查到某人是壞人 → 就唱衰「他力挺的獸首」、把大家往別的獸首帶。全程只談獸首,不談任何人的身分。`;
  }
  return '';
}
...
- **絕對不要說出或暗示「任何人」是什麼角色或哪個陣營**(像「你是內鬼」「他一定是好人」「我覺得某某是老朝奉」都禁止)。你要支持誰、質疑誰,一律只透過「哪些獸首該保、哪些可疑」來表達,不要直接評論別人的身分。
...
export function buildVotePrompts(ctx: VoteContext): { system: string; user: string } {
  ...
  const aim =
    ctx.camp === 'GOOD'
      ? '你要把票投在「你相信是真品」的獸首上,保護真品(好人保住真品才得分)。除了你自己鑑定到的,也可從聊天中「誰的說法比較可信、誰在硬帶風向」去推測哪些是真品——但這只是你心裡的推理,不用說出來。'
